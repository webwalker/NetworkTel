...
public class Caller extends BroadcastReceiver {
	...
	public void onReceive(final Context context, Intent intent) {
		Loggers.d(AppContext.TAG, "------calling------");
		String intentAction = intent.getAction();
		String number = getResultData();
		Boolean force = false;
		if (!AppContext.isAppCall()) {
			AppContext.setAppCall(true);
			setResultData(number);
			Loggers.d(AppContext.TAG, "sim calling.");
			return;
		}
		...
		if (intentAction.equals(Intent.ACTION_NEW_OUTGOING_CALL)
				&& number != null) {
			setResultData(null);
			intent.setClass(context, CallSelectorActivity.class);
			intent.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
			intent.putExtra("number", number);
			context.startActivity(intent);
			Loggers.d(AppContext.TAG, "choosing call type.");
			return;
		}
		...
		if (intentAction.equals(Intent.ACTION_NEW_OUTGOING_CALL)
			...
			if (last_number != null && last_number.equals(number)
					&& (SystemClock.elapsedRealtime() - last_time) < 3000) {
				setResultData(null);
				return;
			}
			...
			if (sip_type && !force) {
				String sExPat = PreferenceManager.getDefaultSharedPreferences(
						context).getString(Settings.PREF_EXCLUDEPAT,
			...
			if (!sip_type) {
				...
				if (number != null
					...
					// Migrate the "prefix" option. TODO Remove this code in a
					// future release.
					...
					if (sp.contains("prefix")) {
						...
						if (!prefix.trim().equals("")) {
							editor.putString(Settings.PREF_SEARCH, "(.*),"
									+ prefix + "\\1");
						}
					...
					if (!ask
							...
							&& PreferenceManager.getDefaultSharedPreferences(
									context).getBoolean(Settings.PREF_PAR,
									Settings.DEFAULT_PAR)) {
						/*
						 * String orig =
						 * intent.getStringExtra("android.phone.extra.ORIGINAL_URI"
						 * ); if (orig.lastIndexOf("/phones") >= 0) { orig =
						 * orig.substring(0,orig.lastIndexOf("/phones")+7); Uri
						 * contactRef = Uri.parse(orig);
						 */
						...
						Cursor phonesCursor = context.getContentResolver()
								.query(contactRef, PHONES_PROJECTION, null,
										null, Phones.ISPRIMARY + " DESC");
					...
					if (PreferenceManager
							.getDefaultSharedPreferences(context)
							.getString(Settings.PREF_PREF,
									Settings.DEFAULT_PREF)
							.equals(Settings.VAL_PREF_SIPONLY))
						force = true;
					if (!ask && Receiver.engine(context).call(number, force))
						setResultData(null);
					else if (!ask
							&& PreferenceManager.getDefaultSharedPreferences(
									context).getBoolean(Settings.PREF_CALLTHRU,
									Settings.DEFAULT_CALLTHRU)
							&& (callthru_prefix = PreferenceManager
									.getDefaultSharedPreferences(context)
									.getString(Settings.PREF_CALLTHRU2,
											Settings.DEFAULT_CALLTHRU2))
									.length() > 0) {
						callthru_number = (callthru_prefix + ","
								+ callthru_number + "#");
	...
	boolean isExcludedType(Vector<Integer> vExTypesCode, String sNumber,
			Context oContext) {
		Uri contactRef = Uri.withAppendedPath(
				Contacts.Phones.CONTENT_FILTER_URL, sNumber);
		final String[] PHONES_PROJECTION = new String[] { People.Phones.NUMBER, // 0
				People.Phones.TYPE, // 1
		};
		Cursor phonesCursor = oContext.getContentResolver().query(contactRef,
