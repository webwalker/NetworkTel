...
public class SipdroidEngine implements RegisterAgentListener {
	...
	UserAgentProfile getUserAgentProfile(String suffix) {
		...
		user_profile.username = SipContext.getUserName();		
		...
		user_profile.passwd = SipContext.getPassword();
		if (PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings.PREF_DOMAIN+suffix, Settings.DEFAULT_DOMAIN).length() == 0) {
			...
			user_profile.realm = SipContext.getIp();
		} else {
	...
	void setOutboundProxy(SipProvider sip_provider,int i) {
		try {
			if (sip_provider != null) sip_provider.setOutboundProxy(new SocketAddress(
					IpAddress.getByName(SipContext.getIp()),
					Integer.valueOf(SipContext.getPort())));
			//PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings.PREF_DNS+i, Settings.DEFAULT_DNS)
			//PreferenceManager.getDefaultSharedPreferences(getUIContext()).getString(Settings.PREF_PORT+(i!=0?i:""), Settings.DEFAULT_PORT))
		} catch (Exception e) {
		}
	}
